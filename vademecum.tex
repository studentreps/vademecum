...
Only who have Gestione Separata can ask for this benefit, that consists in a monthly allowance of the 75\% of the average month income of the recipient, up to a big enough maximum. If you do the request within 8 days from the end of the PhD contract (that can be, and usually is, before the PhD discussion!), INPS will give start to the benefit 8 days after the end of the contract. Otherwise the benefit is calculated from the day after the application is completed. The maximum duration of DIS-COLL is 12 month.


If you do not have Gestiona Separata, this is the first step to do. You must have it if you are a PhD student (italian law requirement).
The procedure for asking the benefit must be done online after logging in into the INPS website with, among the options, SPID or the electronic ID card (CIE -- carta d'identit\`a elettronica). The link of the DIS-COLL request is \href{https://www.inps.it/prestazioni-servizi/dis-coll-indennita-mensile-di-disoccupazione}{https://www.inps.it/prestazioni-servizi/dis-coll-indennita-mensile-di-disoccupazione}
...
Walking along the left wing of the fourth floor, you can find a gray door between the washroom on the right-hand side of the corridor. This room is dedicated to those people who need a place to collect their thoughts, either for pursing their religious faith or meditate. The door should be always open, in case you can always ask for the key at the reception.
...
SISSA is officially closing at 8.00 pm. Only ''presidio tecnico`` is taking care of the SISSA building after this time. Most of the people in charge of dealing with an eventual emergency are not present, so if you want to stay inside the building after 8.00 pm, you have to write an email to \href{mailto:presidio@sissa.it}{presidio@sissa.it} writing the time that you are expected to leave so that they know how many people they have to look after in case of emergency.
...
ARDiS offers students the opportunity to access different canteen areas (including SISSA's one) and their services at a reduced fee (which ranges from 2,30 to 5,10 euros, depending on your ISEE, for the full meal) thanks to ARDiS Canteen Card.
The ARDiS procedure is completely online and to access the canteen's discount you will need a mobile device where you have to install an app. No physical Canteen Card will be issued anymore, but you will have to use a mobile device and an app.

\textbf{Note:} the price reduction depends on the income range (see next section).
...
It happens often that TPL is late in activating the convention with ARDiS, that usually starts a little bit later than expected.
...
After that, you are ready to install the app bookAmeal on your mobile device. Then press the button "Associazione APP Accesso Mensa". You'll have to frame the QR code that shows up on your computer from the APP, then the system is ready to be used in the canteen. 
...
\textbf{Note:} It is not possible to use the app bookAmeal to pay in the SISSA canteen. The pay service can be used only for the canteen listed on ARDiS website, since an agreement between ARDiS and the company that cooks the meals is needed.
